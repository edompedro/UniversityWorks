...
def thread_function(vec1,start,end):
    global result
    transposta = []
    for i in range(start,end):  #linha
        for c in range(len(vec1[i])):   #coluna
            transposta.append(vec1[c][i])
        result.append(transposta)
...
if __name__ == "__main__":
    ...
    for id in range(size_threads):
        ...
        t = threading.Thread(target=thread_function, args=(matriz1, start, end)) #inicializa a thread, informa o nome da função e os parâmetros
